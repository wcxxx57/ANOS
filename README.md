...
**吴晨曦**：完成了 **进程与文件系统的交互 (proc.c)**、 **ELF 程序加载器 (exec.c)**、**扩充系统调用接口 (syscall)**，以及对应部分的README文档

**丁熙妍**：完成了 **目录项（Dentry）的高级功能**、**文件抽象层（File）**与**设备管理（Device）**的实现，以及对应部分的README文档
...
- console.c以及uart.c和print.c的修改：为了支持console控制台交互，我们引入了 **行缓冲 (Line Buffering)** 机制，并对uart.c和print.c进行了相应的小修改
...
- 用户态测试架构的梳理 (`src/user`, `Makefile`, `mkfs`)：
  ...
  我们深入分析了用户程序的构建与运行链条，明确了本次实验的测试逻辑：
...
设备文件不占用磁盘数据块，而是通过 **主设备号 (Major)** 映射到内核中的函数指针表。我们构建了一个**通用的设备驱动框架**，使得内核可以**像操作文件一样操作硬件或虚拟设备**。
...
在完成了文件系统的底层构建后，我们需要将其与进程管理模块深度绑定，使进程拥有“**打开的文件表**“和“**当前工作目录**”的概念。
...
为了支持这两个字段的**生命周期**，我们在 `proc.c`修改了以下关键函数：
...
其中需要注意的是**proc_return的设置**，在写的时候我们考虑了几个问题：
...
为了让用户程序能够使用文件系统和进程管理的功能，我们在 `sysfunc.c` 中补充实现了以下系统调用：
...
- **“一切皆文件”的设计哲学** 
  ...
  通过引入 `file_t` 结构体和设备驱动框架，我们成功屏蔽了底层物理资源（磁盘 inode、串口 buffer、虚拟设备）的差异。对于用户程序而言，无论是读写文件、向终端打印字符，还是操作零设备，都统一简化为了 `read/write` 系统调用。这种高度的抽象极大地降低了用户态编程的复杂度，是操作系统中非常智慧的设计哲学
...
- **exec：OS真正"活"了起来!**
  ...
  - **进程管理**赋予了**生命**的流转（Trapframe 上下文切换）。
   ...
   在此之前，文件只是磁盘上冰冷的字节，进程只是内存中重复的克隆（fork）；是 `exec` 打通了二者，让静态的程序变成了动态运行的进程，让我们的 OS 真正“活”了起来！
...
从基础构建，到~~非常难的~~进程模块，再到最后的文件系统和整合，我们了解了操作系统如何一步一步启动，从hello world到全系统整合，从单进程到多进程，从内核到用户，如何处理中断，如何管理内存，如何分层抽象...终于建立了自己的小型OS内核

感谢助教的辛勤付出！（助教发布作业的仓库：[ECNU-OSLab-2025-Task: 从0到1的小型内核实现 ](https://gitee.com/xu-ke-123/ecnu-oslab-2025-task/tree/master/)）

感谢石老师的理论课为我们的实践打下基础！

感谢靠谱队友的愉快合作！（贡献者：[dxy831 (Ding Xiyan)](https://github.com/dxy831)，[wcxxx57 (Wu Chenxi)](https://github.com/wcxxx57)）
