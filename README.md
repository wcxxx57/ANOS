...
我选择了一种**轻量级、无阻塞、低延迟**的同步机制 — **自旋锁**来解决输出混乱的问题。


### 4.1 自旋锁接口的实现（spinlock.c）

首先，要实现一个锁，必须要有**锁的状态**，而本实验中已经定义好了`spinlock_t`结构体，其中不仅考虑了是否上锁，还考虑了**锁的持有者**，来保证锁的状态完整性。

因此在`spinlock_holding` 中，不仅需要检查锁是否被持有，还需要验证**持有者是否是当前cpu**，防止出现一个cpu错误释放了其他cpu持有的锁的问题。

接着实现锁的获取与释放的核心逻辑：**原子性**。

为了防止多个cpu同时操作锁，获取与释放锁这两个操作必须是**原子**的：

- **获取锁**：使用**原子操作**`__sync_lock_test_and_set(&lk->locked, 1)`
  它将`lk->locked` 的值设置为 `1`，并返回设置前的旧值。
  
  具体流程为：

```mermaid
graph TD
    A[开始获取锁] --> B{尝试原子地将 locked 设为 1};
    B -- 返回 0 (成功) --> C[成功获取锁];
    B -- 返回 1 (失败) --> D{继续循环等待};
    D --> B;
```

- **释放锁**：使用**原子操作**`__sync_lock_release(&lk->locked)`
  它将 `lk->locked` 的值设置为 `0`，让其他等待的cpu可以获取该锁。

但目前我还没有用上已经实现好的 `push_off` 和 `pop_off` 函数，这两个函数在这里有什么作用呢？为什么我们需要这两个函数？

因为**中断**可能导致**死锁**问题。目前可能发生：cpu 0获取了锁，正在执行临界区代码；此时发生了中断，cpu 0开始执行中断处理程序；但如果中断处理程序也尝试**获取同一把锁**，那么cpu 0就会出现**自己等自己**的死锁问题。

而已有的`push_off`和`pop_off`通过**嵌套式中断管理**为我很好的解决了这个问题： 

我只需要在**获取锁前**使用`push_off`**关闭中断**，**释放锁后**使用`pop_off`**开启中断**，恢复原始中断状态，就能避免死锁的发生。

然后，为了防止编译器优化和CPU乱序执行可能会**重新排列指令**，导致锁的作用失效，我使用了 `__sync_synchronize` 作为**内存屏障**，来保证**临界区代码不会逃离锁的保护**。

最后，为了及时发现编程错误，避免错误自动发生导致更严重的系统故障，我使用了 `assert`输出错误信息：

 - 在 `spinlock_acquire` 中断言**当前CPU未持有该**，防止递归获取导致死锁。
 - 在 `spinlock_release` 中断言**只有锁的持有者才能释放它**，防止应该cpu误释放其他cpu持有的锁。
  
综上成功实现了`spinlock.c`中的`spinlock_init`、`spinlock_acquire`、`spinlock_release`和 `spinlock_holding`四个核心接口。
