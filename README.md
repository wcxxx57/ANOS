...
我们在 `locate_or_add_block` 函数中实现了三级映射逻辑，并支持**按需分配**（即当逻辑块不存在时，自动分配物理块并清零）。

具体的映射逻辑如下图所示：

```mermaid
graph TD
    Inode[Inode Index Array]
    L0[Direct Mapping<br/>0-9]
    L1[Level 1 Indirect<br/>10-11]
    L2[Level 2 Indirect<br/>12]
    
    Data0[Data Block]
    Index1[L1 Index Block]
    Index2[L2 Index Block]
    Index1_Sub[L1 Index Block]
    Data_Final[Data Block]

    Inode -->|index 0..9| Data0
    Inode -->|index 10..11| Index1
    Inode -->|index 12| Index2
    
    Index1 -->|entry 0..1023| Data_Final
    Index2 -->|entry 0..1023| Index1_Sub
    Index1_Sub -->|entry 0..1023| Data_Final

    style Inode fill:#f9f,stroke:#333,stroke-width:2px
    style Data0 fill:#ccf,stroke:#333
    style Index1 fill:#cfc,stroke:#333
    style Index2 fill:#fcf,stroke:#333
```
...
*   采用深度优先策略，先释放叶子节点（数据块），再释放中间节点（索引块）。
*   `__free_data_blocks` 函数接收当前块号和层级（level），如果 `level > 0`，则读取该块内容并递归调用自身释放子块，最后释放当前块。

### 2. dentry.c

对于用户而言，记忆数字形式的 `inode_num` 是痛苦的。文件系统通过 **目录（Directory）** 和 **目录项（Dentry）** 建立了“文件名 -> Inode”的映射。

#### 目录项管理 (Dentry Management)

目录本质上是一种特殊的文件（`INODE_TYPE_DIR`），其数据块中存储的是 `dentry_t` 结构体数组。我们实现了以下核心操作：
...
*   **`dentry_create`**：
    ...
    *   如果目录块未分配，则自动分配并初始化。
    *   写入新的文件名和 Inode 编号，并更新目录大小。
*   **`dentry_delete`**：
    *   找到目标目录项，将其 `name` 清零，标记为空闲。
    *   这实现了文件的“解引用”（Unlink），当 Inode 的引用计数归零时，文件才会被真正删除。

#### 路径解析系统 (Path Resolution)

为了支持 `/home/user/file.txt` 这样的绝对路径访问，我们实现了路径解析逻辑 `__path_to_inode`。

*   **解析流程**：
    1.  从根目录（`ROOT_INODE`）开始。
    2.  利用 `get_element` 函数逐级提取路径分量（如从 `/a/b` 中提取 `a`）。
    3.  调用 `dentry_search` 在当前目录中查找下一级 Inode。
    4.  循环直到路径结束。
*   **Hand-over-hand Locking (交替加锁)**：
    *   为了保证并发安全，在从父目录切换到子目录时，我们严格遵守锁的顺序：
        1.  持有父目录锁。
        2.  查找子目录 Inode。
        3.  **释放父目录锁**。
        4.  获取子目录锁。
    *   这种策略避免了长时间持有上层目录的锁，提高了并发性能，同时也防止了死锁。
*   **父目录查找**：
    *   `path_to_parent_inode` 用于 `mkdir` 或 `create` 操作。
    *   当解析到路径的最后一级时（如 `/a/b` 中的 `b`），它会提前停止，返回父目录 `a` 的 Inode 以及最后一级的文件名 `b`。
    *   **修复点**：在返回父节点前，必须显式调用 `inode_unlock`，否则会导致调用者再次加锁时发生死锁。

## 测试与验证

我们在 `fs_init` 中集成了四个阶段的测试用例，全面验证了文件系统的功能。

### Test 1: Inode 基础操作
验证了 Inode 的创建、查找、引用计数管理（`dup`/`put`）以及位图分配的正确性。

### Test 2: 大文件读写与多级索引
通过写入超过直接映射范围的数据（触发一级和二级间接索引），验证了 `locate_or_add_block` 的动态分配逻辑和 `inode_write_data` 的正确性。
*   **修复**：修正了助教提供的测试用例中关于 `inode_read_data` 读取长度的错误，确保了测试的严谨性。

### Test 3: 目录项操作
验证了 `dentry_create`、`dentry_search` 和 `dentry_delete`。我们成功在根目录下创建了新目录，并验证了重名检测和删除逻辑。

### Test 4: 路径解析
验证了 `path_to_inode` 解析多级路径（如 `///AABBC///aaabb/file.txt`）的能力，正确处理了路径中的冗余斜杠，并成功读取了深层文件的内容。
*   **修复**：修复了 `path_to_parent_inode` 在找到父节点返回时**忘记解锁**的 Bug。如果不解锁，测试代码在后续调用 `inode_lock## 具体实现
